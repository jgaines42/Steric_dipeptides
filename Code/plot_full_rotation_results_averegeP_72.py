...
all_max_60 = np.zeros([72, 72])
all_max_180 = np.zeros([72, 72])
all_max_300 = np.zeros([72, 72])
all_Chi_pos = [0] * 72
all_probs = [[0] * 72 for i in range(72)]
...
save_tag = '042221'
prob_spots = 0
save_this_chi = np.zeros([72*72, 72])
for run_loop in range(0, runs):
    ...
    for phi_loop in range(0, 72):
        for psi_loop in range(0, 72):
            this_data = Val_data[counter:counter + 72, :]
            counter = counter + 72
...
run_max_60 = np.zeros([72, 72])
run_max_180 = np.zeros([72, 72])
run_max_300 = np.zeros([72, 72])

this_60 = np.zeros([72, 72])
this_180 = np.zeros([72, 72])
this_300 = np.zeros([72, 72])
...
for phi_loop in range(0, 72):
    for psi_loop in range(0, 72):
        ...
        phi = phi + 36
        psi = psi + 36
        all_60[psi, phi] = np.sum(this_data[0:24])
        all_180[psi, phi] = np.sum(this_data[24:48])
        all_300[psi, phi] = np.sum(this_data[48:72])

        max1 = np.max(this_data[0:24])
        result = np.where(this_data[0:24] == max1)
        result = result[0]
        ind_max = result[int(len(result) / 2)]

        if (all_60[psi, phi] / runs > 0.1):
            all_max_60[psi, phi] = ind_max * 5
            if (ind_max * 5 == 0):
        ...
        max1 = np.max(this_data[24:48])
        result = np.where(this_data[24:48] == max1)
        result = result[0]
        ind_max = result[int(len(result) / 2)]

        if (all_180[psi, phi] / runs > 0.1):
            all_max_180[psi, phi] = (ind_max + 24) * 5
            if (ind_max * 5 == 0):
        ...
        max1 = np.max(this_data[48:72])
        result = np.where(this_data[48:72] == max1)
        result = result[0]
        ind_max = result[int(len(result) / 2)]
        if (all_300[psi, phi] / runs > 0.1):
            all_max_300[psi, phi] = (ind_max + 48) * 5
        ...
        # min_2 = this_data[6:36].min() / runs
        # min_3 = this_data[36:30].min() / runs
        # min_4 = this_data[30:72].min() / runs
...
np.savetxt(folder + 'Val_steric_60_' + save_tag + '_all' + str(runs) + '_new.txt', all_60)
np.savetxt(folder + 'Val_steric_180_' + save_tag + '_all' + str(runs) + '_new.txt', all_180)
np.savetxt(folder + 'Val_steric_300_' + save_tag + '_all' + str(runs) + '_new.txt', all_300)
np.savetxt(folder + 'Val_steric_60_max_' + save_tag + '_all' + str(runs) + '_new.txt', all_max_60)
np.savetxt(folder + 'Val_steric_180_max_' + save_tag + '_all' + str(runs) + '_new.txt', all_max_180)
np.savetxt(folder + 'Val_steric_300_max_' + save_tag + '_all' + str(runs) + '_new.txt', all_max_300)
